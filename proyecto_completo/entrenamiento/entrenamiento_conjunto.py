...
def entrenar_gan_cnn_conjunto(generador_datos_gan, input_dim, num_epochs=10):
    ...
    for epoch in range(num_epochs):
        for paso in range(pasos_por_epoca):
            ...
            etiquetas_reales = np.ones((tamano_lote, 2))
            etiquetas_falsas = np.zeros((tamano_lote, 2))
            ...
            # Concatenar datos generados buenos y malos
            datos_concatenados = np.concatenate([datos_generados_buenos, datos_generados_malos])

            # Etiquetas para el modelo completo
            etiquetas_modelo_completo = np.concatenate([etiquetas_reales, etiquetas_falsas], axis=1)

            # Entrenar el modelo completo (generadores)
            modelo_completo.train_on_batch(ruido, etiquetas_modelo_completo)
